...
static ModeData[E_EASY_DEATHMATCH];

enum E_EASY_DEATHMATCH_PLAYER
{
	ORM: ORM_ID,
	Score,
	Wins,
...
static PlayerModeData[MAX_PLAYERS][E_EASY_DEATHMATCH_PLAYER];

forward _EasyDeathmatch();

Hook:EDM_OnGameModeExit()
{
...
Hook:EDM_initModule()
{
	this = RegisterMode("Easy Deathmatch", "EDM", 481, 64);
...
Hook:EDM_OnPlayerRegister(playerid)
{
	new query[128];
	mysql_format(g_Sql, query, sizeof(query), "INSERT INTO module_easy_deathmatch (uid) VALUES (%d)", Player[playerid][ID]);
	mysql_pquery(g_Sql, query);
	return true;
}

Hook:EDM_OnPlayerJoinModeHook(playerid, newmode, oldmode)
{
	if (newmode == this)
	{
		...
		static const empty_data[E_EASY_DEATHMATCH_PLAYER];
		PlayerModeData[playerid] = empty_data;

		new ORM: ormid = PlayerModeData[playerid][ORM_ID] = orm_create("module_easy_deathmatch", g_Sql);
...
Hook:EDM_OnPlayerModeDataLoaded(playerid, gamemodeid)
{
	if (gamemodeid == this)
	{
...
Hook:EDM_CreateModeSpawnpoint(modelid,Float:X,Float:Y,Float:Z,Float:rZ,gamemodeid)
{
	if (gamemodeid == this)
		ModeData[Spawns]++;
	return true;
...
Hook:EDM_InitiatePlayerSpawns(gamemodeid)
{
	if (gamemodeid == this)
	{
		RevertFade(this);
...
Hook:EDM_OnModeRequestToptimes(gamemodeid)
{
	if (gamemodeid == this)
	{
		for (new i; i < 5; i++)
...
Hook:EDM_OnCountDownStateChange(gamemodeid,cdstate)
{
	if (gamemodeid == this)
	{
		switch (cdstate)
...
Hook:EDM_RequestMapRedo(playerid, gamemodeid)
{
	if (gamemodeid == this)
	{
...
Hook:EDM_OnPlayerReachHunter(playerid, gamemodeid)
{
...
Hook:EDM_RequestVirtualWorld(playerid)
{
...
Hook:EDM_OPRequestHuntertime(playerid, tick)
{
...
Hook:EDM_OnPlayerSetHuntertime(playerid, tick)
{
	if (Player[playerid][ModeID] == this)
	{
		cache_set_result(1);
...
Hook:EDM_OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
...
Hook:EDM_OnPlayerDeath(playerid, killerid, reason)
{
	if (Player[playerid][ModeID] == this && PlayerModeData[playerid][IsAlive])
		AddPlayerToDeathlist(playerid, GetTickCount() - ModeData[StartTick]);
	return true;
...
Hook:EDM_OnPlayerSetNextmap(playerid, rowcount, mapname[], mapauthor[], mapid, bool:adminset)
{
	if (Player[playerid][ModeID] == this)
	{
		cache_set_result(0);
...
Hook:EDM_OnQueueContinues(gamemodeid)
{
	if (gamemodeid == this)
	{
		new rows = cache_num_rows();
